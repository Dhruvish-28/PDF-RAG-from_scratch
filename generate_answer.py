...
def generate_answer(query, chunks , chunk_embedding, index, convo):
    """
    parameters:
    ...
    passes all things to retrieve.py to get top 3 relevant chunks and scores.
    Then control goes to prompt_builder to obtain prompt to be given to LLM
    At last LLM generates response
    """
    
    selected_chunks = retrieval(query, chunks , chunk_embedding, index, convo)

    prompt,score = build_prompt(selected_chunks, query,convo)
